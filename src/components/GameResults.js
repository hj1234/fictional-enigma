  ...
  return (
    <div className="h-screen bg-black text-amber-500 font-mono p-3 md:p-6 flex items-center justify-center overflow-hidden">
      <div className="max-w-5xl w-full h-full flex flex-col">
        {/* Header */}
        <div className="text-center mb-1.5 md:mb-2 shrink-0">
          <h1 className="text-base md:text-lg font-bold text-amber-500 mb-0.5">POD SHOP</h1>
          <h2 className="text-sm md:text-base font-bold text-white mb-0.5">{results.fund_name}</h2>
          <p className="text-[10px] text-gray-500">FINAL RESULTS</p>
        </div>

        {/* Stats Grid */}
        <div className="grid grid-cols-2 gap-1.5 md:gap-2 mb-2 md:mb-2.5 shrink-0">
          <div className="bg-gray-900 border border-amber-800 p-1.5 md:p-2 rounded">
            <div className="text-[9px] md:text-[10px] text-gray-500 uppercase mb-0.5">Lifetime PnL</div>
            <div className={`text-sm md:text-lg font-bold ${results.total_pnl >= 0 ? 'text-green-500' : 'text-red-500'}`}>
              {fmtMoney(results.total_pnl)}
            </div>
          </div>
          <div className="bg-gray-900 border border-amber-800 p-1.5 md:p-2 rounded">
            <div className="text-[9px] md:text-[10px] text-gray-500 uppercase mb-0.5">Firm Cash</div>
            <div className={`text-sm md:text-lg font-bold ${firmCash >= 0 ? 'text-amber-500' : 'text-red-500'}`}>
              {fmtMoney(firmCash)}
            </div>
          </div>
          <div className="bg-gray-900 border border-amber-800 p-1.5 md:p-2 rounded">
            <div className="text-[9px] md:text-[10px] text-gray-500 uppercase mb-0.5">Days since inception</div>
            <div className="text-sm md:text-lg font-bold text-gray-300">
              {results.game_days_played !== null && results.game_days_played !== undefined ? results.game_days_played : '—'}
            </div>
          </div>
          <div className="bg-gray-900 border border-amber-800 p-1.5 md:p-2 rounded">
            <div className="text-[9px] md:text-[10px] text-gray-500 uppercase mb-0.5">Annualised performance</div>
            <div className={`text-sm md:text-lg font-bold ${results.annualized_performance !== null && results.annualized_performance !== undefined && results.annualized_performance >= 0 ? 'text-green-500' : results.annualized_performance !== null && results.annualized_performance !== undefined ? 'text-red-500' : 'text-gray-500'}`}>
        ...
        <div className="bg-gray-900 border border-amber-800 p-2 md:p-3 rounded mb-2 md:mb-2.5 flex-[2] min-h-0 flex flex-col">
        ...
        {/* Share Section */}
        <div className="bg-gray-900 border border-amber-800 p-1.5 md:p-2 rounded mb-1.5 md:mb-2 shrink-0">
